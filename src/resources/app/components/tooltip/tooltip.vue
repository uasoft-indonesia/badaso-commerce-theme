<template>
  <div :class="cssClass">
    <div ref="trigger" class="trigger" style="display: inline-block" :aria-describedby="isOpen ? popoverId : undefined" :tabindex="trigger.indexOf('focus') !== -1 ? 0 : undefined">
      <slot />
    </div>

    <div v-if="isOpen" :id="popoverId" ref="popover" :class="[popoverBaseClass, popoverClass, cssClass, 'shadow-md bg-white border p-2 transition-all']" :style="{ visibility: isOpen ? 'visible' : 'hidden' }" :aria-hidden="isOpen ? 'false' : 'true'" :tabindex="autoHide ? 0 : undefined" @keyup.esc="autoHide && hide()">
      <div :class="popoverWrapperClass">
        <div ref="inner" :class="popoverInnerClass" style="position: relative">
          <div>
            <slot name="popover" :is-open="isOpen" />
          </div>

          <ResizeObserver v-if="handleResize" @notify="$_handleResize" />
        </div>
        <div ref="arrow" :class="popoverArrowClass" />
      </div>
    </div>
  </div>
</template>
