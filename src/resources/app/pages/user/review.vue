<template>
  <div>
              ...
              <div class="w-full flex flex-wrap gap-2">
                ...
                <div class="w-full flex items-start justify-start">
                  <label class="inline-flex text-center gap-2 select-none clear-both" :for="`media-${index}`">
                    ...
                    <template v-if="media[index] && media[index].length <= 5">
    ...
    <div class="sm:hidden">
          ...
          <div :key="index" class="w-full">
            <Link :href="route('badaso.commerce-theme.detail', orderDetail.productDetail.product.slug)" class="flex space-x-4 w-full py-4 items-start" :key="`product-${index}`">
              ...
              <div class="flex flex-col gap-1">
                ...
                <div class="text-gray-500 text-xs">Variant: {{ orderDetail.productDetail.name }}</div>
                <div class="text-xs text-gray-600">x{{ orderDetail.quantity }}</div>
                <div class="flex flex-nowrap gap-1">
                  <div class="text-gray-300 mr-2 text-sm line-through" v-if="orderDetail.discounted">{{ $currency(orderDetail.price) }}</div>
                  <div class="text-primary text-sm">{{ $currency(orderDetail.price - orderDetail.discounted) }}</div>
                </div>
              </div>
              <div class="flex-grow" />
            </Link>
          ...
          <div :key="`review-${index}`" class="w-full flex-wrap flex gap-4 mb-4">
            <div class="w-full flex flex-wrap gap-2">
              <div class="w-full flex justify-center">
                <rating stroke interactable v-model="rating[index]" star-active-color="rgba(6, 187, 211, 1)" star-empty-color="transparent" class="w-1/2" />
              </div>
              ...
              <div class="w-full flex items-start justify-start">
                <label class="inline-flex text-center gap-2 select-none clear-both" :for="`media-${index}`">
                  <template v-for="m, i in media[index]">
                    <img :src="m" v-if="m.match('data:image/*')" class="h-24 w-24 object-cover" :key="`${index}-${i}`">
                    <video v-if="m.match('data:video/*')" class="h-24 w-24" :key="`${index}-${i}`">
                      <source :src="m">
                    </video>
                  </template>
                  <template v-if="media[index] && media[index].length <= 5">
                    <div class="inline-block w-24 h-24 border-2 border-gray-700 text-gray-700 border-dashed cursor-pointer">
                        <div class="flex justify-center flex-col items-center h-full">
                          <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                          </svg>
                          <div class="text-center text-xs">Tambah Media (maks. 5)</div>
                        </div>
                    </div>
                  </template>
                </label>
      ...
      <div class="flex w-full flex-col justify-center items-center py-4 text-primary bg-white" v-else>
        <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
        <span>Kamu sudah me-<i>review</i> orderan ini.</span>
